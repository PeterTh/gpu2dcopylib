 ...
 *
 * @brief `BS::thread_pool`: a fast, lightweight, modern, and easy-to-use C++17/C++20/C++23 thread pool library. This header file contains the entire library,
 * and is the only file needed to use the library.
 */

#ifndef BS_THREAD_POOL_HPP
#define BS_THREAD_POOL_HPP

// We need to include <version> since if we're using `import std` it will not define any feature-test macros, including `__cpp_lib_modules`, which we need to
// check if `import std` is supported in the first place.
#ifdef __has_include
...
// If the macro `BS_THREAD_POOL_IMPORT_STD` is defined, import the C++ Standard Library as a module. Otherwise, include the relevant Standard Library header
// files. This is currently only officially supported by MSVC with Microsoft STL and LLVM Clang (NOT Apple Clang) with LLVM libc++. It is not supported by GCC
// with any standard library, or any compiler with GNU libstdc++. We also check that the feature is enabled by checking `__cpp_lib_modules`. However, MSVC
// defines this macro even in C++20 mode, which is not standards-compliant, so we check that we are in C++23 mode; MSVC currently reports `__cplusplus` as
...
// Only allow importing the `std` module if the library itself is imported as a module. If the library is included as a header file, this will force the program
// that included the header file to also import `std`, which is not desirable and can lead to compilation errors if the program `#include`s any Standard Library
// header files.
...
static_assert(
    thread_pool_version == version(BS_THREAD_POOL_MODULE), "The versions of BS.thread_pool.cppm and BS_thread_pool.hpp do not match. Aborting compilation.");
/**
...
struct [[nodiscard]] pr_task {
	 ...
	 */
	explicit pr_task(task_t&& task_, const priority_t priority_ = 0) noexcept(std::is_nothrow_move_constructible_v<task_t>)
...
#define BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)                                                                                                                    \
	typename F, typename = std::enable_if_t < std::is_invocable_v<F> || std::is_invocable_v < F, std::size_t >> // NOLINT(bugprone-macro-parentheses)
#endif
...
class [[nodiscard]] multi_future : public std::vector<std::future<T>> {
  public:
	 ...
	 * for the first future for the desired duration. If that future is ready before the duration expires, this function waits for the second future for
	 * whatever remains of the duration. It continues similarly until the duration expires.
	 *
	...
	/**
	 * @brief Wait for all the futures stored in this `BS::multi_future`, but stop waiting after the specified time point has been reached. This function first
 ...
 * which would result in a deadlock. Only used if the flag `BS:tp::wait_deadlock_checks` is enabled in the template parameter of `BS::thread_pool`.
 */
...
/**
 * @brief Get the processor affinity of the current process using the current platform's native API. This should work on Windows and Linux, but is not possible
 * on macOS as the native API does not allow it.
 *
 * @return An `std::optional` object, optionally containing the processor affinity of the current process as an `std::vector<bool>` where each element
 * corresponds to a logical processor. If the returned object does not contain a value, then the affinity could not be determined. On macOS, this function
...
/**
 * @brief Set the processor affinity of the current process using the current platform's native API. This should work on Windows and Linux, but is not possible
 * on macOS as the native API does not allow it.
 *
 ...
 *
 * @return An `std::optional` object, optionally containing the priority of the current process, as a member of the enum `BS::os_process_priority`. If the
 * returned object does not contain a value, then either the priority could not be determined, or it is not one of the pre-defined values and therefore cannot
 * be represented in a portable way.
 */
...
#elif defined(__linux__) || defined(__APPLE__)
	// On Linux/macOS there is no direct analogue of `GetPriorityClass()` on Windows, so instead we get the "nice" value. The usual range is -20 to 19 or 20,
	// with higher values corresponding to lower priorities. However, we are only using 6 pre-defined values for portability, so if the value was set via any
	// means other than `BS::set_os_process_priority()`, it may not match one of our pre-defined values. Note that `getpriority()` returns -1 on error, but
	// since this does not correspond to any of our pre-defined values, this function will return `std::nullopt` anyway.
	const int nice_val = getpriority(PRIO_PROCESS, static_cast<id_t>(getpid()));
	switch(nice_val) {
	case static_cast<int>(os_process_priority::idle): return os_process_priority::idle;
	case static_cast<int>(os_process_priority::below_normal): return os_process_priority::below_normal;
	case static_cast<int>(os_process_priority::normal): return os_process_priority::normal;
	case static_cast<int>(os_process_priority::above_normal): return os_process_priority::above_normal;
	case static_cast<int>(os_process_priority::high): return os_process_priority::high;
	case static_cast<int>(os_process_priority::realtime): return os_process_priority::realtime;
...
/**
 * @brief Set the priority of the current process using the current platform's native API. This should work on Windows, Linux, and macOS. However, note that
 ...
 * @return `true` if the priority was set successfully, `false` otherwise. Usually, `false` means that the user does not have the necessary permissions to set
...
#elif defined(__linux__) || defined(__APPLE__)
	// On Linux/macOS there is no direct analogue of `SetPriorityClass()` on Windows, so instead we set the "nice" value. The usual range is -20 to 19 or 20,
	// with higher values corresponding to lower priorities. However, we are only using 6 pre-defined values for portability. Note that the "nice" values are
	// only relevant for the `SCHED_OTHER` policy, but we do not set that policy here, as it is per-thread rather than per-process. Also, it's important to note
	// that a non-root user cannot decrease the nice value (i.e. increase the process priority), only increase it. This can cause confusing behavior. For
	// example, if the current priority is `BS::os_process_priority::normal` and the user sets it to `BS::os_process_priority::idle`, they cannot change it back
...
class [[nodiscard]] this_thread {
  ...
  public:
	/**
	 * @brief Get the index of the current thread. If this thread belongs to a `BS::thread_pool` object, the return value will be an index in the range `[0, N)`
	 * where `N == BS::thread_pool::get_thread_count()`. Otherwise, for example if this thread is the main thread or an independent thread not in any pools,
	...
	/**
	 * @brief Get a pointer to the thread pool that owns the current thread. If this thread belongs to a `BS::thread_pool` object, the return value will be a
	 * `void` pointer to that object. Otherwise, for example if this thread is the main thread or an independent thread not in any pools, `std::nullopt` will be
	 * returned.
	 *
	 * @return An `std::optional` object, optionally containing a pointer to a thread pool. Note that this will be a `void` pointer, so it must be cast to the
	 * desired instantiation of the `BS::thread_pool` template in order to use any member functions.
	 */
	[[nodiscard]] static std::optional<void*> get_pool() noexcept { return my_pool; }

#ifdef BS_THREAD_POOL_NATIVE_EXTENSIONS
	/**
	 * @brief Get the processor affinity of the current thread using the current platform's native API. This should work on Windows and Linux, but is not
	 * possible on macOS as the native API does not allow it.
	 *
	 * @return An `std::optional` object, optionally containing the processor affinity of the current thread as an `std::vector<bool>` where each element
	 * corresponds to a logical processor. If the returned object does not contain a value, then the affinity could not be determined. On macOS, this function
	 * always returns `std::nullopt`.
	 */
	[[nodiscard]] static std::optional<std::vector<bool>> get_os_thread_affinity() {
#if defined(_WIN32)
		// Windows does not have a `GetThreadAffinityMask()` function, but `SetThreadAffinityMask()` returns the previous affinity mask, so we can use that to
		// get the current affinity and then restore it. It's a bit of a hack, but it works. Since the thread affinity must be a subset of the process affinity,
		// we use the process affinity as the temporary value.
		DWORD_PTR process_mask = 0;
		DWORD_PTR system_mask = 0;
		if(GetProcessAffinityMask(GetCurrentProcess(), &process_mask, &system_mask) == 0) return std::nullopt;
	...
	/**
	 * @brief Set the processor affinity of the current thread using the current platform's native API. This should work on Windows and Linux, but is not
	 * possible on macOS as the native API does not allow it. Note that the thread affinity must be a subset of the process affinity (as obtained using
	 ...
	 *
	 * @return An `std::optional` object, optionally containing the name of the current thread. If the returned object does not contain a value, then the name
	 ...
	 * thread names are limited to 16 characters, including the null terminator.
	 *
	 ...
	 *
	 * @return An `std::optional` object, optionally containing the priority of the current thread, as a member of the enum `BS::os_thread_priority`. If the
	 * returned object does not contain a value, then either the priority could not be determined, or it is not one of the pre-defined values.
	 */
	[[nodiscard]] static std::optional<os_thread_priority> get_os_thread_priority() {
...
#elif defined(__linux__)
		// On Linux, we distill the choices of scheduling policy, priority, and "nice" value into 7 pre-defined levels, for simplicity and portability. The
		// total number of possible combinations of policies and priorities is much larger, so if the value was set via any means other than
		...
		if(policy == SCHED_OTHER) {
			// For SCHED_OTHER, the result depends on the "nice" value. The usual range is -20 to 19 or 20, with higher values corresponding to lower
			// priorities. Note that `getpriority()` returns -1 on error, but since this does not correspond to any of our pre-defined values, this function
			// will return `std::nullopt` anyway.
			const int nice_val = getpriority(PRIO_PROCESS, static_cast<id_t>(syscall(SYS_gettid)));
			switch(nice_val) {
			case PRIO_MIN + 2: return os_thread_priority::above_normal;
			case 0: return os_thread_priority::normal;
			case(PRIO_MAX / 2) + (PRIO_MAX % 2): return os_thread_priority::below_normal;
			case PRIO_MAX - 3: return os_thread_priority::lowest;
#ifdef __APPLE__
			// `SCHED_IDLE` doesn't exist on macOS, so we use the policy `SCHED_OTHER` with a "nice" value of `PRIO_MAX - 2`.
			case PRIO_MAX - 2: return os_thread_priority::idle;
#endif
			default: return std::nullopt;
			}
		}
		return std::nullopt;
#elif defined(__APPLE__)
		// On macOS, we distill the choices of scheduling policy and priority into 7 pre-defined levels, for simplicity and portability. The total number of
		// possible combinations of policies and priorities is much larger, so if the value was set via any means other than
		// `BS::this_thread::set_os_thread_priority()`, it may not match one of our pre-defined values.
		int policy = 0;
		struct sched_param param = {};
		if(pthread_getschedparam(pthread_self(), &policy, &param) != 0) return std::nullopt;
		...
		if(policy == SCHED_OTHER) {
			...
			if(param.sched_priority
			    == sched_get_priority_min(SCHED_OTHER) + (sched_get_priority_max(SCHED_OTHER) - sched_get_priority_min(SCHED_OTHER)) * 2 / 3)
	...
	/**
	 * @brief Set the priority of the current thread using the current platform's native API. This should work on Windows, Linux, and macOS. However, note that
	 * higher priorities might require elevated permissions.
	 *
	 * @param priority The priority to set. Must be a value from the enum `BS::os_thread_priority`.
	 * @return `true` if the priority was set successfully, `false` otherwise. Usually, `false` means that the user does not have the necessary permissions to
	...
	static bool set_os_thread_priority(const os_thread_priority priority) {
...
#elif defined(__linux__)
		// On Linux, we distill the choices of scheduling policy, priority, and "nice" value into 7 pre-defined levels, for simplicity and portability. The
		// total number of possible combinations of policies and priorities is much larger, but allowing more fine-grained control would not be portable.
		int policy = 0;
		struct sched_param param = {};
		...
		case os_thread_priority::above_normal:
			// "Above normal" pre-defined priority: We use the policy `SCHED_OTHER` (the default). This policy does not accept a priority value, so priority
			// must be 0. However, we set the "nice" value to the minimum value as given by `PRIO_MIN`, plus 2 (which should evaluate to -18). The usual range
			// is -20 to 19 or 20, with higher values corresponding to lower priorities.
			policy = SCHED_OTHER;
		...
		case os_thread_priority::below_normal:
			// "Below normal" pre-defined priority: We use the policy `SCHED_OTHER`, priority must be 0, and we set the "nice" value to half the maximum value
			// as given by `PRIO_MAX`, rounded up (which should evaluate to 10).
			policy = SCHED_OTHER;
			param.sched_priority = 0;
			nice_val = (PRIO_MAX / 2) + (PRIO_MAX % 2);
			break;
		case os_thread_priority::lowest:
			// "Lowest" pre-defined priority: We use the policy `SCHED_OTHER`, priority must be 0, and we set the "nice" value to the maximum value as given by
			// `PRIO_MAX`, minus 3 (which should evaluate to 17).
			policy = SCHED_OTHER;
			param.sched_priority = 0;
			nice_val = PRIO_MAX - 3;
			break;
		...
		return success;
#elif defined(__APPLE__)
		// On macOS, unlike Linux, the "nice" value is per-process, not per-thread (in compliance with the POSIX standard). However, unlike Linux, `SCHED_OTHER`
		// on macOS does have a range of priorities. So for `realtime` and `highest` priorities we use `SCHED_FIFO` and `SCHED_RR` respectively as for Linux,
		// but for the other priorities we use `SCHED_OTHER` with a priority in the range given by `sched_get_priority_min(SCHED_OTHER)` to
		...
		case os_thread_priority::above_normal:
			...
			break;
		case os_thread_priority::normal:
			// "Normal" pre-defined priority: We use the policy `SCHED_OTHER` (the default) with a priority in the middle of the available range (which appears
			// to be the default?).
			policy = SCHED_OTHER;
...
struct common_index_type<T1, T2, std::enable_if_t<(std::is_signed_v<T1> && std::is_unsigned_v<T2>) || (std::is_unsigned_v<T1> && std::is_signed_v<T2>)>> {
	...
	using type = std::conditional_t<larger_size <= 4,
	    // If both integers are 32 bits or less, the common type should be a signed type that can hold both of them. If both are 8 bits, or the signed type is
	    // 16 bits and the unsigned type is 8 bits, the common type is `std::int16_t`. Otherwise, if both are 16 bits, or the signed type is 32 bits and the
	    // unsigned type is smaller, the common type is `std::int32_t`. Otherwise, if both are 32 bits or less, the common type is `std::int64_t`.
	    ...
	    // If the unsigned integer is 64 bits, the common type should also be an unsigned 64-bit integer, that is, `std::uint64_t`. The reason is that the most
	    // common scenario where this might happen is where the indices go from 0 to `x` where `x` has been previously defined as `std::size_t`, e.g. the size
	    // of a vector. Note that this will fail if the first index is negative; in that case, the user must cast the indices explicitly to the desired common
	    // type. If the unsigned integer is not 64 bits, then the signed integer must be 64 bits, hence the common type is `std::int64_t`.
...
/**
 * @brief A fast, lightweight, modern, and easy-to-use C++17/C++20/C++23 thread pool class. This alias defines a thread pool with all optional features
 * disabled.
 */
 ...
 *
 * @tparam OptFlags A bitmask of flags which can be used to enable optional features. The flags are members of the `BS::tp` enumeration: `BS::tp::priority`,
 * `BS::tp::pause`, and `BS::tp::wait_deadlock_checks`. The default is `BS::tp::none`, which disables all optional features. To enable multiple features, use
 * the bitwise OR operator `|`, e.g. `BS::tp::priority | BS::tp::pause`.
 */
...
class [[nodiscard]] thread_pool {
  public:
	 ...
	 *
	 * @param init An initialization function to run in each thread before it starts executing any submitted tasks. The function must have no return value, and
	 * can either take one argument, the thread index of type `std::size_t`, or zero arguments. It will be executed exactly once per thread, when the thread is
	 * first constructed. The initialization function must not throw any exceptions, as that will result in program termination. Any exceptions must be handled
	...
	/**
	 * @brief Destruct the thread pool. Waits for all tasks to complete, then destroys all threads. If a cleanup function was set, it will run in each thread
	 * right before it is destroyed. Note that if the pool is paused, then any tasks still in the queue will never be executed.
	 */
	 ...
	 * block function takes two arguments, the start and end of the block, so that it is only called once per block, but it is up to the user make sure the
	 * block function correctly deals with all the indices in each block. Does not return a `BS::multi_future`, so the user must use `wait()` or some other
	 ...
	 * threads in the pool.
	 * @param priority The priority of the tasks. Should be between -128 and +127 (a signed 8-bit integer). The default is 0. Only taken into account if the
	 * flag `BS:tp::priority` is enabled in the template parameter, otherwise has no effect.
	 */
	...
	/**
	 * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue, with the specified priority. The
	 * loop function takes one argument, the loop index, so that it is called many times per block. Does not return a `BS::multi_future`, so the user must use
	...
	/**
	 * @brief Submit a sequence of tasks enumerated by indices to the queue, with the specified priority. The sequence function takes one argument, the task
	 * index, and will be called once per index. Does not return a `BS::multi_future`, so the user must use `wait()` or some other method to ensure that the
	...
	/**
	 * @brief Submit a function with no arguments and no return value into the task queue, with the specified priority. To submit a function with arguments,
	 ...
	 * @param priority The priority of the task. Should be between -128 and +127 (a signed 8-bit integer). The default is 0. Only taken into account if the flag
	 * `BS:tp::priority` is enabled in the template parameter, otherwise has no effect.
	 */
	template <typename F>
	 ...
	 * `std::thread::native_handle()` (or `std::jthread::native_handle()` in C++20 and later).
	 *
	...
	/**
	 * @brief Get a vector containing the unique identifiers for each of the pool's threads, as obtained by `std::thread::get_id()` (or `std::jthread::get_id()`
	 * in C++20 and later).
	 *
	 ...
	 * until they are finished. Only enabled if the flag `BS:tp::pause` is enabled in the template parameter.
	 */
	BS_THREAD_POOL_IF_PAUSE_ENABLED
	void pause() {
		const std::scoped_lock tasks_lock(tasks_mutex);
	 ...
	 *
	 * @param cleanup A cleanup function to run in each thread right before it is destroyed, which will happen when the pool is destructed or reset. The
	 * function must have no return value, and can either take one argument, the thread index of type `std::size_t`, or zero arguments. The cleanup function
	 * must not throw any exceptions, as that will result in program termination. Any exceptions must be handled explicitly within the function.
	 */
	template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
	 ...
	 * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue, with the specified priority. The
	 * block function takes two arguments, the start and end of the block, so that it is only called once per block, but it is up to the user make sure the
	 * block function correctly deals with all the indices in each block. Returns a `BS::multi_future` that contains the futures for all of the blocks.
	 *
	 * @tparam T1 The type of the first index. Should be a signed or unsigned integer.
	 * @tparam T2 The type of the index after the last index. Should be a signed or unsigned integer.
	 * @tparam F The type of the function to loop through.
	 ...
	 * @param priority The priority of the tasks. Should be between -128 and +127 (a signed 8-bit integer). The default is 0. Only taken into account if the
	 * flag `BS:tp::priority` is enabled in the template parameter, otherwise has no effect.
	 * @return A `BS::multi_future` that can be used to wait for all the blocks to finish. If the block function returns a value, the `BS::multi_future` can
	...
	[[nodiscard]] multi_future<R> submit_blocks(
	    const T1 first_index, const T2 index_after_last, F&& block, const std::size_t num_blocks = 0, const priority_t priority = 0) {
		if(static_cast<T>(index_after_last) > static_cast<T>(first_index)) {
			const std::shared_ptr<std::decay_t<F>> block_ptr = std::make_shared<std::decay_t<F>>(std::forward<F>(block));
			const blocks blks(static_cast<T>(first_index), static_cast<T>(index_after_last), num_blocks ? num_blocks : thread_count);
			...
			for(std::size_t blk = 0; blk < blks.get_num_blocks(); ++blk) {
				future.push_back(submit_task([block_ptr, start = blks.start(blk), end = blks.end(blk)] { return (*block_ptr)(start, end); }, priority));
			}
			return future;
		}
		return {};
	...
	/**
	 * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue, with the specified priority. The
	 * loop function takes one argument, the loop index, so that it is called many times per block. It must have no return value. Returns a `BS::multi_future`
	 ...
	 * be submitted, and an empty `BS::multi_future` will be returned.
	...
	[[nodiscard]] multi_future<void> submit_loop(
	    const T1 first_index, const T2 index_after_last, F&& loop, const std::size_t num_blocks = 0, const priority_t priority = 0) {
		if(static_cast<T>(index_after_last) > static_cast<T>(first_index)) {
			const std::shared_ptr<std::decay_t<F>> loop_ptr = std::make_shared<std::decay_t<F>>(std::forward<F>(loop));
			const blocks blks(static_cast<T>(first_index), static_cast<T>(index_after_last), num_blocks ? num_blocks : thread_count);
	...
	/**
	 * @brief Submit a sequence of tasks enumerated by indices to the queue, with the specified priority. The sequence function takes one argument, the task
	 * index, and will be called once per index. Returns a `BS::multi_future` that contains the futures for all of the tasks.
	 *
	 * @tparam T1 The type of the first index. Should be a signed or unsigned integer.
	 * @tparam T2 The type of the index after the last index. Should be a signed or unsigned integer.
	 * @tparam F The type of the function used to define the sequence.
	 ...
	 * @param priority The priority of the tasks. Should be between -128 and +127 (a signed 8-bit integer). The default is 0. Only taken into account if the
	 * flag `BS:tp::priority` is enabled in the template parameter, otherwise has no effect.
	 * @return A `BS::multi_future` that can be used to wait for all the tasks to finish. If the sequence function returns a value, the `BS::multi_future` can
...
#define BS_THREAD_POOL_PROMISE_MEMBER_ACCESS promise->
	...
	/**
	 * @brief Unpause the pool. The workers will resume retrieving new tasks out of the queue. Only enabled if the flag `BS:tp::pause` is enabled in the
	 * template parameter.
	 */
	BS_THREAD_POOL_IF_PAUSE_ENABLED
	void unpause() {
	 ...
	 *
	 * @throws `wait_deadlock` if called from within a thread of the same pool, which would result in a deadlock. Only enabled if the flag
  ...
  private:
	 ...
	 * will be created with the total number of hardware threads available, as obtained from `thread_t::hardware_concurrency()`. If the latter returns zero for
	...
	/**
	 * @brief Reset the pool with a new number of threads and a new initialization function. This member function implements the actual reset, while the public
	 * member function `reset()` also handles the case where the pool is paused.
	 *
	 * @param num_threads The number of threads to use.
	 * @param init An initialization function to run in each thread before it starts executing any submitted tasks.
	 */
	...
	void worker(BS_THREAD_POOL_WORKER_TOKEN const std::size_t idx) {
		...
		while(true) {
			...
			{
				task_t task = pop_task(); // NOLINT(misc-const-correctness) In C++23 this cannot be const since `std::move_only_function::operator()` is not a
	...
	/**
	 * @brief A cleanup function to run in each thread right before it is destroyed, which will happen when the pool is destructed or reset. The function must
	 * have no return value, and can either take one argument, the thread index of type `std::size_t`, or zero arguments. The cleanup function must not throw
	 * any exceptions, as that will result in program termination. Any exceptions must be handled explicitly within the function. The default is an empty
	 * function, i.e., no cleanup will be performed.
	 */
	function_t<void(std::size_t)> cleanup_func = [](std::size_t) {};

	/**
	 * @brief An initialization function to run in each thread before it starts executing any submitted tasks. The function must have no return value, and can
	 * either take one argument, the thread index of type `std::size_t`, or zero arguments. It will be executed exactly once per thread, when the thread is
	 * first constructed. The initialization function must not throw any exceptions, as that will result in program termination. Any exceptions must be handled
	 * explicitly within the function. The default is an empty function, i.e., no initialization will be performed.
	 */
	 ...
	 * although any tasks already executed will keep running until they are finished. When set to `false` again, the workers resume retrieving tasks. Only
...
class [[nodiscard]] synced_stream {
  public:
	...
	/**
	 * @brief A stream manipulator to pass to a `BS::synced_stream` (an explicit cast of `std::endl`). Prints a newline character to the stream, and then
	 * flushes it. Should only be used if flushing is desired, otherwise a newline character should be used instead.
	 */
...
#else
/**
 * @brief A polyfill for `std::counting_semaphore`, to be used if C++20 features are not available. A `counting_semaphore` is a synchronization primitive that
 * allows more than one concurrent access to the same resource. The number of concurrent accessors is limited by the semaphore's counter, which is decremented
 * when a thread acquires the semaphore and incremented when a thread releases the semaphore. If the counter is zero, a thread trying to acquire the semaphore
 * will be blocked until another thread releases the semaphore.
 *
...
class [[nodiscard]] counting_semaphore {
  ...
  public:
	...
	/**
	 * @brief Atomically decrements the internal counter by 1 if it is greater than 0; otherwise blocks until it is greater than 0 and can successfully
	...
	/**
	 * @brief Atomically increments the internal counter. Any thread(s) waiting for the counter to be greater than 0, such as due to being blocked in
	...
	/**
	 * @brief Tries to atomically decrement the internal counter by 1 if it is greater than 0; otherwise blocks until it is greater than 0 and can successfully
