...
bool validate_contents(executor& exec, device_id did, intptr_t buffer, int64_t offset, size_t bytes, uint32_t start_value) {
	...
	auto queue = exec.get_queue(did);
	auto* valid_ptr = sycl::malloc_shared<bool>(1, queue);
	queue.parallel_for(sycl::range<1>{count}, [=](sycl::id<1> idx) {
		*valid_ptr &= ptr[idx[0]] == start_value + static_cast<uint32_t>(idx[0]);
		if(!*valid_ptr) { printf("Mismatch at index %ld: expected %u, got %u\n", idx[0], start_value + static_cast<uint32_t>(idx[0]), ptr[idx[0]]); }
	});
	queue.wait_and_throw();
...
TEST_CASE("basic copies can be executed", "[executor]") {
	...
	const auto d0_buffer = reinterpret_cast<intptr_t>(exec.get_buffer(device_id::d0));
	const data_layout source_layout{d0_buffer, 0, buffer_size};
	fill_source(exec, device_id::d0, d0_buffer, 0, buffer_size, 42);

	const auto d1_buffer = reinterpret_cast<intptr_t>(exec.get_buffer(device_id::d1));
	const data_layout target_layout{d1_buffer, 0, buffer_size};
	...
	CAPTURE(props);
	const copy_spec spec{device_id::d0, source_layout, device_id::d1, target_layout, props};
	execute_copy(exec, normalize(spec));

	CHECK(validate_contents(exec, device_id::d1, d1_buffer, 0, buffer_size, 42));
}
...
TEST_CASE("2D copies can be executed", "[executor]") {
	...
	auto d0_buffer = reinterpret_cast<intptr_t>(exec.get_buffer(device_id::d0));
	...
	const data_layout source_layout{d0_buffer, source_offset, source_frag_length, source_frag_count, source_stride};

	fill_uniform(exec, device_id::d0, d0_buffer, buffer_size, 77);
	for(int i = 0; i < source_frag_count; i++) {
		fill_source(exec, device_id::d0, d0_buffer, source_layout.fragment_offset(i), source_layout.fragment_length, 42 + i * 100);
	}

	constexpr bool debug_print = false;
	if(debug_print) {
		// print source buffer contents for debugging, as uint32_t
		auto ptr = reinterpret_cast<uint32_t*>(d0_buffer);
		for(size_t i = 0; i < source_layout.end_offset() / sizeof(uint32_t); i++) {
			printf("src %lu %p %u\n", i, ptr + i, ptr[i]);
		}
	}

	auto d1_buffer = reinterpret_cast<intptr_t>(exec.get_buffer(device_id::d1));
	...
	const data_layout target_layout{d1_buffer, target_offset, target_frag_length, target_frag_count, target_stride};

	fill_uniform(exec, device_id::d1, d1_buffer, buffer_size, 66);
	...
	if(debug_print) {
		auto ptr = reinterpret_cast<uint32_t*>(d1_buffer);
	...
	{
		auto ptr = reinterpret_cast<uint32_t*>(d1_buffer + target_offset);
		auto queue = exec.get_queue(device_id::d1);
		auto* valid_ptr = sycl::malloc_shared<bool>(1, queue);
		...
		queue.parallel_for(sycl::range<1>{elem_count}, [=](sycl::id<1> idx) {
			...
			if(frag_offset < target_frag_length && frag_idx < target_frag_count) {
				...
				const bool valid = ptr[id] == expected;
				*valid_ptr &= valid;
